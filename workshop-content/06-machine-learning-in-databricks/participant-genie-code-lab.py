...
# MAGIC %md
# MAGIC # Section 06 — FPD5 Genie Code Lab
# MAGIC
...
# MAGIC
# MAGIC > **Runnable companion to `genie-code-runbook.md`** — the same FPD5 model, targeting the canonical `hc_workshop` catalog. Each numbered step matches a prompt/code block in that runbook.
...
# Targets the canonical `hc_workshop` catalog: the synthetic dataset (core_lending)
# and the writable `workshop_labs` schema must exist there. A facilitator updates
# this constant to point the lab at a different workshop catalog.
