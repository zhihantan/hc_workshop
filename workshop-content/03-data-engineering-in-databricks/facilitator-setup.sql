...
-- MAGIC
-- MAGIC - Catalog: `sean_development_catalog` (canonical design value is `hc_workshop`)
...
-- MAGIC
-- MAGIC If the workshop catalog changes, replace every `sean_development_catalog` reference consistently.
...
FROM sean_development_catalog.information_schema.tables
...
-- COMMAND ----------
CREATE OR REPLACE TABLE sean_development_catalog.workshop_shared.lending_raw_loan_application AS
SELECT
...
FROM sean_development_catalog.core_lending.loan_application;

-- COMMAND ----------
CREATE OR REPLACE TABLE sean_development_catalog.workshop_shared.lending_raw_credit_contract AS
SELECT
  ...
  DATE'2026-09-01' AS batch_date,
  current_timestamp() AS _ingested_at
FROM sean_development_catalog.core_lending.credit_contract;

-- COMMAND ----------
CREATE OR REPLACE TABLE sean_development_catalog.workshop_shared.lending_raw_installment AS
SELECT
  ...
  DATE'2026-09-01' AS batch_date,
  current_timestamp() AS _ingested_at
FROM sean_development_catalog.core_lending.installment;
...
-- GRANT USE CATALOG ON CATALOG sean_development_catalog TO `<participant-group>`;
-- GRANT USE SCHEMA, SELECT ON SCHEMA sean_development_catalog.workshop_shared TO `<participant-group>`;
-- GRANT USE SCHEMA, SELECT ON SCHEMA sean_development_catalog.core_lending TO `<participant-group>`;
-- GRANT CREATE SCHEMA ON CATALOG sean_development_catalog TO `<participant-group>`;  -- each creates their own pipeline target schema
...
INSERT INTO sean_development_catalog.workshop_shared.lending_raw_installment
...
INSERT INTO sean_development_catalog.workshop_shared.lending_raw_loan_application
...
-- MAGIC ## Reset helpers
-- MAGIC ```sql
-- MAGIC DELETE FROM sean_development_catalog.workshop_shared.lending_raw_installment       WHERE batch_date > DATE'2026-09-01';
-- MAGIC DELETE FROM sean_development_catalog.workshop_shared.lending_raw_loan_application   WHERE batch_date > DATE'2026-09-01';
-- MAGIC ```
